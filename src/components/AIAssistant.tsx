...
import { Bot, Send, Key, Sparkles, X, Trash2 } from 'lucide-react';
...
export function AIAssistant({ isOpen, onClose, selectedDate }: AIAssistantProps) {
  ...
  const handleSend = () => {
    ...
    sendMessage(inputVal, selectedDate);
    setInputVal('');
  };
        ...
        {!geminiKey ? (
          ...
          <>
            ...
            <div className="p-3 bg-card border-t border-border flex gap-2">
              <button onClick={() => { if(window.confirm("Clear all logs?")) clearChats(); }} className="p-3 text-muted-foreground hover:text-destructive transition-colors rounded-xl">
                <Trash2 size={18}/>
              </button>
              <input 
                type="text"
                ...
                placeholder={`Tell AI to log something for ${selectedDate.getDate()}...`}
                className="flex-1 bg-muted/50 border border-border rounded-xl px-4 text-sm focus:outline-none focus:border-primary/50 text-foreground"
              />
              <button 
                onClick={handleSend}
